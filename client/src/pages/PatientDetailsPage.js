      ...
      <div
                  ...
                  return (
                    <div key={key}>
                      <ListItem>
                        <ListItemAvatar>
                          <Avatar>
                            {item.replace(/(?:_| |\b)(\w)/g, function(str, p1) {
                              return p1.toUpperCase();
                            })}
                          </Avatar>
                        </ListItemAvatar>
                        <ListItemText
                          primary={item.replace(/(?:_| |\b)(\w)/g, function(str, p1) {
                            return p1.toUpperCase();
                          })}
                          secondary={
                            item === 'createdAt' || item === 'updatedAt'
                              ? new Date(patientInfo[0][item]).toLocaleString()
                              : patientInfo[0][item]
                          }
                        />
                      </ListItem>
        ...
        <Card sx={{ marginLeft: '1rem', flexDirection: 'row' }}>
          <Typography variant="h5">
            Exam Info: {examInfo.length} {examInfo.length > 1 ? 'exams' : 'exam'}
          </Typography>
          <Divider />
                ...
                <div key={index}>
                  ...
                  <Paper variant="outlined">
                    ...
                    {examInfo[index].image ? (
                      ...
                      <BrokenImageIcon
                        sx={{ width: '300px', height: '200px', backgroundColor: 'gray' }}
                      />
                    )}
                    <Stack
                      direction="row"
                      ...
                      {examInfo[index].score.map((item, key) => {
                        return <Chip key={key} label={item} color={getColor(item)} />;
                      })}
                    </Stack>
                    <Box>
